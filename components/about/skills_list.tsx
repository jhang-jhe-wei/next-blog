      ...
      <ul className="mt-10 grid grid-cols-11 gap-8">
        {
          data.map((skills, index)=> (
            <li key={skills.title} className={`py-9 px-10 bg-white ${(index % 3 == 0)? "col-span-5": "col-span-6"}`}>
              <h3 className="text-2xl font-bold text-cyan-450">{skills.title}</h3>
              <div className="flex mt-5">
                <ul>
                  {
                    skills.skills.map(skill => (
                      <li key={skill.name} className="mt-5 text-right">
                        {skill.name}
                      </li>
                    ))
                  }
                </ul>
                <ul className="flex-1 ml-5 -translate-y-1">
                  {
                    skills.skills.map(skill => (
                      <li key={skill.name} className="mt-8">
                        <div className="h-2.5 bg-gradient-to-r from-red-500 to-yellow-400 rounded-xl" style={{width: `${skill.proficiency}%`}}></div>
                      </li>
                    ))
                  }
                </ul>
